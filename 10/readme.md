...
- 从汇编器到后端编译器再到前端编译器，输入解析部分的复杂都增加了一个层次。
    ...
    后端编译器的输入是中间语言构成的**逐行的**代码序列，对输入的解析仍然相对简单，复杂的部分主要是在输出时需要将抽象的中间代码翻译为对应的汇编指令。
...
在上述例子中， $\text{FIRST}(S) = \lbrace(, a\rbrace, \text{FIRST}(F) = \lbrace a\rbrace, \text{FOLLOW}(S)=\lbrace +, \\$ \rbrace, \text{FOLLOW}(F)=\lbrace+\rbrace$
...
- 解析器的主要工作是完成词法单元的切分和类型识别。
    ...
    - Tokenizer如果使用正则逃课则较为简单，可以使用Python的re库来实现[Tokenizer](./Python/JackTokenizer.py)。

    - 注意注释的语法有三种：`//`、`/* */`和`/** */`，其中多行注释的匹配自然包含了API注释`/** */`的匹配。

- 本项目从本章开始选择使用C++（Cpp17）实现编译器前端部分（我们已经在自己实现编程语言了，这里选择C++是为了练习C++的语法和标准库）。
